...
import { FIELD_TYPE_CONFIG, FIELD_TYPES } from '@/lib/fieldTypes';
...
export default function FieldTypeMenu() {
    return (
        <div className='bg-[#eeeeee] flex flex-row pt-6 border-r border-gray-300 text-sm h-full'>
            <div className='flex-1 flex flex-col h-full min-w-40 xl:min-w-60'>
                ...
                {FIELD_TYPES.map((field) => {
                    const config = FIELD_TYPE_CONFIG[field]
                    const Icon = ICON_MAP[config.icon]

                    return (
                        <button className='cursor-pointer hover:border-y-1 hover:border-stone-200'>
